...
const getHighlight = (highlight) => {
  if (highlight) {
    if (highlight == "green") return styles.highlight_green;
    else if (highlight == "blue") return styles.highlight_blue;
  }
...
const ProjectDescription = ({ project, className }) => {
  ...
  return (
    <Div
      align="stretch"
      className={`${styles.description_container} ${className}`}
    >
      ...
      {map(description, (description, index) => {
        if (description.type == "text") {
          return (
            <div
              key={index}
              className={`${styles.text} ${getHighlight(
          ...
          return (
            <Div
              key={index}
              className={`${styles.points_container} ${getHighlight(
                description.highlight
              )}`}
            >
              {description.title ? (
